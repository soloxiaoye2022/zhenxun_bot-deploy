...
Download_zhenxun_bot() {
 ...
 while true; do
    ...
    if [ $? = 0 ] ; then
     ...
     echo "${Info} zhenxun_bot下载失败，3秒后重试下载"
  fi
  sleep 3
 done
 ...
 while true; do
  gocq_version=$(wget -O- -t1 -T2 "https://api.github.com/repos/Mrs4s/go-cqhttp/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
   ...
   if [ $? = 0 ] ; then
      ...
      echo "${Error} go-cqhttp 下载失败，3秒后重试下载"
      sleep 3
   fi
 done
}
...
echo -e "${Info} 请输入要选择的apt源,默认为北外源
  ${Green_font_prefix} 1.${Font_color_suffix} 北外
  ...
  ${Green_font_prefix} 5.${Font_color_suffix} 163 (广东电信：适合电信用户)"
  read -erp "请输入数字 [1-5], 默认为 1:" select_source
