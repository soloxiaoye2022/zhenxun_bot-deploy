...
update_shell_url="https://gitee.com/soloxiaoye/zhenxun_bot_tool/attach_files/1092670/download/install.sh"
...
Download_zhenxun_bot() {
    ...
    echo -e "${Info} 如果使用代理下载速度仍然很慢，请键入Ctrl键+C停止进程，重新使用脚本进行安装"
    ...
    cd "${WORK_DIR}" || exit 1
    mv "${TMP_DIR}/zhenxun_bot" "${WORK_DIR}"
    mkdir -p "go-cqhttp"
    tar -zxf "${TMP_DIR}/go-cqhttp.tar.gz" -C "${WORK_DIR}"/go-cqhttp/
    echo -e "${Info} 开始下载抽卡相关资源...下载时间较长,请耐心等待越10分钟..."
...
Install_zhenxun_bot() {
    ...
    echo -e "${Info} 开始安装poetry依赖 ..."
    cd ${WORK_DIR}/zhenxun_bot
    poetry install
    echo -e "${Info} 安装poetry依赖结束 ..."
    endTime=`date +%s`
