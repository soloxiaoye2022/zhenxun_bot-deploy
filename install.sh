...
local_dns=$(cat /etc/resolv.conf | grep "nameserver" | head -n 1 | awk '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
 if [[ $local_dns == $dns ]];then
     ...
     echo -e "${Info} 是否修改 dns ?(大陆地区不修改可能会有各种蜜汁网络问题)"
...
Set_sync_dir() {
   ...
   if [ ! $INODE_NUM == '2' ]; then
    if [[ ! -d /sd/bot/ ]]; then
     mkdir /sd/bot/ /sd/bot/go-cqhttp/ /sd/bot/zhenxun_bot/
     sync_zhenxun_dir
    else
     sync_zhenxun_dir
    fi
     if [[ ! -d ${GOCQ_DIR} ]]; then
       mkdir ${GOCQ_DIR}
       echo -e "${Tip} ${GOCQ_DIR} 不存在!"
     else
       sync_gocq_dir
     fi
     
   fi
   echo -e "${lnfo} 创建同步目录:手机内置储存/download/bot. "
}
...
INODE_NUM=`ls -ali / | sed '2!d' |awk {'print $1'}`
   if [ ! $INODE_NUM == '2' ]; then
     cd /root/zhenxun_bot/
      if [[ ! -d $path ]]; then
        path=/sd/bot/zhenxun_bot
        mkdir /sd/bot/ $path /sd/bot/zhenxun_bot/extensive_plugin/
        ln -s /sd/bot/zhenxun_bot/extensive_plugin/ .
     fi
...
sync_gocq_dir() {
     cd /root/go-cqhttp/
     mv config.yml device.json session.token qrcode.png /sd/bot//go-cqhttp/
     ln -s /sd/bot/go-cqhttp/config.yml config.yml
     ln -s ${SYNC_DIR}/go-cqhttp/device.json device.json
     ln -s ${SYNC_DIR}/go-cqhttp/session.token session.token
     ln -s ${SYNC_DIR}/go-cqhttp/qrcode.png qrcode.png
}
...
then
      ...
      postgresql \
      postgresql-contrib 
      sudo cat > /home/sql.sql <<-EOF
CREATE USER zhenxun WITH PASSWORD 'zxpassword';
CREATE DATABASE zhenxun OWNER zhenxun;
EOF  
      cd /home && su postgres -c "psql -f sql.sql"
...
sudo cat > ${TMP_DIR}/sql.sql <<-EOF
CREATE USER zhenxun WITH PASSWORD 'zxpassword';
CREATE DATABASE zhenxun OWNER zhenxun;
EOF
    su postgres -c "psql -f ${TMP_DIR}/sql.sql"
    echo -e "${lnfo} 数据库用户名zhenxun,密码zxpassword,数据库名:zhenxun"
}
...
menu_termux() {
 ...
 ————————————
 ${Green_font_prefix} 4.${Font_color_suffix} 创建 同步目录
